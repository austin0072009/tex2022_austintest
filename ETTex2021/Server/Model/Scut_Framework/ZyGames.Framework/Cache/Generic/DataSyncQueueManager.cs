...
using ETModel.Framework.Profile;
    ...
    public abstract class DataSyncQueueManager
    {
        ...
        private static void OnEntitySyncQueue(object state)
        {
            if (Interlocked.CompareExchange(ref _entityQueueRunning, 1, 0) == 0)
            {
                ...
                {
                    TraceLog.WriteError("201810271110. EntitySync queue {0},----{1}", ex, JsonUtils.Serialize(_entitySet));
                }
                finally
                {
            ...
            foreach (var g in typeGroup)
            {
                ...
                try
                {
                    ...
                    if (type == null)
                    {
                        ...
                        if (type == null)
                        {
                            throw new Exception(string.Format("Get entity \"{0}\" type is null", entityTypeName));
                        }
                    }
