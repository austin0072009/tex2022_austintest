...
using ETModel.Framework.Net.Sql;
...
namespace ETModel.Framework.Cache.Generic
{
    ...
    public abstract class BaseCacheStruct<T> : BaseDisposable where T : AbstractEntity, new()
    {
        ...
        public T TryGetFromDB(DbDataFilter filter, string personalId = "")
        {
            return TryGetFromDB(SchemaTable(), filter);
        }

        public T TryGetFromDB(long value)
        {
            var schema = SchemaTable();
            if (schema != null && schema.Keys != null && schema.Keys.Length == 1)
            {
                return TryGetFromDB(schema, new DbDataFilter(1) { Condition = schema.Keys[0] + "=" + value });
            }
            return null;
        }

        public T TryGetFromDB(SchemaTable schema, DbDataFilter filter)
        {
            using (IDataReceiver getter = new SqlDataReceiver(schema, filter))
            {
                if (getter.TryReceive(out List<T> dataList) && dataList.Count > 0) return dataList[0];
            }
            return null;
        }
