...
namespace ETModel
{
	...
	public class KChannel : AChannel
	{
		...
		// accept
		public KChannel(uint localConn, uint remoteConn, Socket socket, IPEndPoint remoteEndPoint, KService kService) : base(kService, ETChannelType.Accept)
		{
		...
		// connect
		public KChannel(uint localConn, Socket socket, IPEndPoint remoteEndPoint, KService kService) : base(kService, ETChannelType.Connect)
		{
			this.memoryStream = this.GetService().MemoryStreamManager.GetStream("message", ushort.MaxValue);
		...
		public void Update()
		{
			...
			if (!this.isConnected)
			{
				...
				switch (ChannelType)
				{
					case ETChannelType.Accept:
						this.Accept();
						break;
					case ETChannelType.Connect:
						this.Connect();
						break;
				}
