...
namespace ETModel
{
    ...
    public class RDBCommonFun3RequestHandler : AMRpcHandler<DB_CommonFun3Request, DB_CommonFun3Reply>
    {
        ...
        public RDBCommonFun3RequestHandler()
        {
        ...
        protected override async ETTask Run(Session session, DB_CommonFun3Request request, DB_CommonFun3Reply response, Action reply)
        {
            ...
            try
            {
                ...
                {
                    response.Message = JsonUtils.Serialize(mp.Key.Invoke(null, CommonFun.GetParams(mp.Value, request.para1, request.para2, request.para3)));
                }
                else
                {
                ...
                //object[] args = CommonFun.GetParams(alltryre.GetParameters(), request.para1, request.para2, request.para3);
                //object _tobj = alltryre.Invoke(null, args);
                //response.Message = JsonUtils.Serialize(_tobj);
            }
            catch (Exception ex)
            {
                TraceLogEx.Error(ex, "202009133 request.funname：" + request.funname);
