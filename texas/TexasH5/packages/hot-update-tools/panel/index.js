        ...
        window.plugin = new window.Vue({
            ...
            methods: {
                ...
                _packageVersion () {
                    ...
                    //打包子游戏manifest的版本文件
                    let gamesmanifest = path.join(this.resourceRootDir, "Games");
                    this._packageDir(gamesmanifest, jszip.folder("Games"));
                    ...
                    let packZipName = "ver_" + this.version + ".zip",
                ...
                _genVersion (version, serverRootDir, resourceRootDir, genManifestDir) {
                    ...
                    this._readDir(path.join(source, "assets/GameCommon"), manifest.assets, source);
                    this._readDir(path.join(source, "assets/gameHall"), manifest.assets, source);
                    this._readDir(path.join(source, "assets/texas"), manifest.assets, source);
                    ...
                    bulidPathManifestDir = `${resourceRootDir}/manifest`;
                    for (let i = 0; i < games.length; i++) {
                        ...
                        if ("/" === serverRootDir[serverRootDir.length - 1]) {
                            submanifest.remoteManifestUrl = `${serverRootDir}Games/${key}/project.manifest`;
                            submanifest.remoteVersionUrl = `${serverRootDir}Games/${key}/version.manifest`;
                        } else {
                            submanifest.remoteManifestUrl = `${serverRootDir}/Games/${key}/project.manifest`;
                            submanifest.remoteVersionUrl = `${serverRootDir}/Games/${key}/version.manifest`;
                        }
                        ...
                        let subgameManifest = path.join(dest, "project.manifest");
                        let subgameVersionManifest = path.join(dest, "version.manifest");
                        this._mkdirSync(dest);

                        this._mkdirSync(`${resourceRootDir}/Games`);
                        //生成构建目录下的manifest文件
                        let bulidPathManifestDir = `${resourceRootDir}/Games/${key}`;
                        if (fs.existsSync(bulidPathManifestDir)) {
                            this._delDir(bulidPathManifestDir);
                        }
                        this._mkdirSync(bulidPathManifestDir);
                        ...
                        subgameManifest = path.join(bulidPathManifestDir, "project.manifest");
                        fs.writeFileSync(subgameManifest, JSON.stringify(submanifest));
                        this._addLog(`[Build] 生成 ${key}/project.manifest成功`);
                        ...
                        subgameVersionManifest = path.join(bulidPathManifestDir, "version.manifest");
                        fs.writeFileSync(subgameVersionManifest, JSON.stringify(submanifest));
                        this._addLog(`[Build] 生成 ${key}/version.manifest成功`);
                    }
                ...
                _updateSubGameServerVersion (gamedirName) {
                    if (this.localServerPath.length > 0) {
                        let path = require("fire-path"),
                            fs = require("fire-fs"),
                            filename = path.join(this.localServerPath, `Games/${gamedirName}/version.manifest`);
                        fs.readFile(filename, "utf-8", function (err, data) {
                            if (err) {
                                let projectManifest = path.join(this.localServerPath, `Games/${gamedirName}/project.manifest`);
                    ...
                    for (let i = 0; i < games.length; i++) {
                        ...
                        if (!this.subGameInclude[game]) {
                            ...
                            if (fs.existsSync(gamePath)) {
                                ...
                                //删除子游戏 版本控制文件
                                let versionManifestPath = path.join(this.resourceRootDir, `Games/${game}/version.manifest`);
                                if (fs.existsSync(versionManifestPath)) {
                                    fs.unlinkSync(versionManifestPath)
                                    this._addLog(`删除子游戏${game} : ${versionManifestPath}`);
                                }

                                let projectManifestPath = path.join(this.resourceRootDir, `Games/${game}/project.manifest`);
