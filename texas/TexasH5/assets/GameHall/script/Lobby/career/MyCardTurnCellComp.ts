...
import { PInfoSD, TexActionSD, TexasTurnActionEnum, TexasTurnEnum, TexUserInfoSD } from './CareerNetDataStruct';
import MyCardHistory from './MyCardHistory';
import { TexasBase } from './TexasBase';
export class MyCardTurnCellComp extends ViewBase {
    public historyComp: MyCardHistory = null;
    ...
    public SetTurnCellInfo(cellItem: fgui.GComponent, infoSD: TexActionSD, folds = 0) {
        ...
        if (folds <= 1) {
            ...
            handleValue.text = UIUtil.formatNumber100(infoSD.g) + "";
            posName.text = TexasBase.getPlayerPosType(infoSD.pos, this.bpos, this.tuInfo);
    ...
    public SetHandleBytType(type: TexasTurnActionEnum, txt: fgui.GTextField, img: fgui.GLoader, curTurn: number) {
        ...
        txt.text = str;
        UIUtil.loadImage(img, imgStr, "Lobby")
    }
...
export class TurnTitleComp extends ViewBase {
    ...
    public DisplayTitle(playerNum: number, cards: number[], turn: number, lastDizhu: number) {
        ...
        this.ui_lastTurnGamble.text = UIUtil.formatNumber100(lastDizhu) + "";
        let go: fgui.GObject = null;
        ...
        for (var i = 0; i < cards.length && i < cardsNum; i++) {
            ...
            UIUtil.loadImage(go.asCom.getChild("turnCardCell").asLoader, cardId == 0 ? "001" : cardId + "_1", "Lobby");
        }
    }
